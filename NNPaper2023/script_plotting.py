...
# NN hist
ax.hist(
    (y_NNregE_pred[idx_nn, 0] - y_regE_true[idx_nn, 0]) / y_regE_true[idx_nn, 0],
    bins=bins,
    alpha=0.2,
    color="red",
)
ax.hist(
    ...
    + r"FWHM = {:.2f} $\pm$ {:.2f}".format(
        poptNN[1] * 2.355, np.sqrt(pcovNN[1, 1]) * 2.355
    ),
)

# CB hist
ax.hist(
    (y_CBregE_pred[idx_cb, 0] - y_regE_true[idx_cb, 0]) / y_regE_true[idx_cb, 0],
    bins=bins,
    alpha=0.2,
    color="black",
)
ax.hist(
    ...
    + "\n"
    + r"FWHM = {:.2f} $\pm$ {:.2f}".format(
        poptCB[1] * 2.355, np.sqrt(pcovCB[1, 1]) * 2.355
    ),
)
"""
