...
def check_compton_kinematics(e, p, ee=0.0, ep=0.0, compton=True):
    if compton:
        ...
        compton_edge = (
            (event_energy / (1 + ELECTRON_MASS / (2 * event_energy))),
            event_energy
            * (ELECTRON_MASS + event_energy)
            / (ELECTRON_MASS / 2 + event_energy)
            / (ELECTRON_MASS / 2 + event_energy)
            * event_energy_uncertainty,
        )
...
def check_DAC(
    e, p, p_ex, p_ey, p_ez, p_px, p_py, p_pz, beam_diff, inverse, return_dac=False
):
    ELECTRON_MASS = 0.511
    ...
    theta = tmath_acos(
        1
        - ELECTRON_MASS * (1 / (photon_energy) - 1 / (photon_energy + electron_energy))
    )
