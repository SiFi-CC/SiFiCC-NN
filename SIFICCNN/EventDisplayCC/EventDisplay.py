...
class PlotCanvas(FigureCanvas):
    ...
    def plot_event(self, event, detector, event_idx, show_sipms=True, show_cluster_area=True, show_compton_hits=True):
        ...
        event.plot(detector, event_idx=event_idx, ax=self.axes, show_sipms=show_sipms,
                   show_cluster_area=show_cluster_area, show_compton_hits=show_compton_hits)
        self.draw()
...
class DatasetViewer(QMainWindow):
    ...
    def __init__(self):
        ...
        self.table_widget.setHorizontalHeaderLabels(
            ["Event Index", "Num Clusters", "Is Compton Event"])
    ...
    def update_table(self):
        ...
        for row, event in enumerate(self.current_block):
            ...
            self.table_widget.setItem(row, 2, NumericTableWidgetItem(
                str(event.contains_non_compton_hit == False)))
    ...
    def plot_event(self, event):
        ...
        self.plot_canvas.plot_event(
            event, self.detector, event_idx, show_sipms, show_cluster_area, show_compton_hits)
    ...
    def populate_table(self):
        ...
        for row, event in enumerate(self.current_block):
            ...
            self.table_widget.setItem(row, 2, QTableWidgetItem(
                "No" if event.contains_non_compton_hit else "Yes"))
