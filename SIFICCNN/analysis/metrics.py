...
def accuracy(tp, fp, tn, fn, weighted=False):
    ...
    if weighted:
        ...
        class_weights = [N / (2 * (tn + fp)),
                         N / (2 * (tp + fn))]

        return ((tp * class_weights[1]) + (tn * class_weights[0])) / \
               (((tp + fp) * class_weights[1]) +
                ((tn + fn) * class_weights[0]))
    else:
        return (tp + tn) / N
...
def print_classifier_summary(y_scores, y_true, run_name=""):
    ...
    # grab base binary classifier evaluation for optimal threshold determined by ROC analysis
    ...
    print(" pos   | {:^9.1f}% | {:^9.1f}% |".format(
        tp / N * 100, fp / N * 100))
    print("-------|------------|------------|")
    print(" Pred. | {:^10d} | {:^10d} |".format(fn, tn))
    print(" neg   | {:^9.1f}% | {:^9.1f}% |".format(
        fn / N * 100, tn / N * 100))
    print("-------|------------|------------|")
    print("")
    ...
    print("Baseline accuracy    : {:.1f} %".format(
        (1 - ((tp + fn) / N)) * 100))
    print("")
    print("# Threshold: {:.3f}".format(0.5))
    print(
        "Efficiency           : {:.2f} +/- {:.2f} %".format(eff * 100, eff_unc * 100))
    print(
        "Purity               : {:.2f} +/- {:.2f} %".format(pur * 100, pur_unc * 100))
    print(
        "Accuracy             : {:.2f} +/- {:.2f} %".format(acc * 100, acc_unc * 100))
    print(
        "Accuracy (weighted)  : {:.2f} +/- {:.2f} %".format(acc_w * 100, acc_w_unc * 100))
    print("")

    print("# Threshold: {:.3f}".format(theta))
    print(
        "Efficiency           : {:.2f} +/- {:.2f} %".format(eff_opt * 100, eff_opt_unc * 100))
    print(
        "Purity               : {:.2f} +/- {:.2f} %".format(pur_opt * 100, pur_opt_unc * 100))
    print(
        "Accuracy             : {:.2f} +/- {:.2f} %".format(acc_opt * 100, acc_opt_unc * 100))
    print(
        "Accuracy (weighted)  : {:.2f} +/- {:.2f} %".format(acc_opt_w * 100, acc_opt_w_unc * 100))
...
def write_classifier_summary(y_scores, y_true, run_name=""):
    ...
    with open("metrics.txt", 'w') as f:
        ...
        f.write(
            "Efficiency           : {:.2f} +/- {:.2f} %\n".format(eff * 100, eff_unc * 100))
        f.write(
            "Purity               : {:.2f} +/- {:.2f} %\n".format(pur * 100, pur_unc * 100))
        f.write(
            "Accuracy             : {:.2f} +/- {:.2f} %\n".format(acc * 100, acc_unc * 100))
        f.write(
            "Accuracy (weighted)  : {:.2f} +/- {:.2f} %\n".format(acc_w * 100, acc_w_unc * 100))
        f.write("\n")
        f.write("# Threshold: {:.3f}\n".format(theta))
        f.write(
            "Efficiency           : {:.2f} +/- {:.2f} %\n".format(eff_opt * 100, eff_opt_unc * 100))
        f.write(
            "Purity               : {:.2f} +/- {:.2f} %\n".format(pur_opt * 100, pur_opt_unc * 100))
        f.write(
            "Accuracy             : {:.2f} +/- {:.2f} %\n".format(acc_opt * 100, acc_opt_unc * 100))
        f.write("Accuracy (weighted)  : {:.2f} +/- {:.2f} %\n".format(acc_opt_w * 100,
                                                                      acc_opt_w_unc * 100))
        f.close()
