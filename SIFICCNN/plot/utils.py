...
def gaussian(x, mu, sigma, A, c):
    return (
        A / (sigma * np.sqrt(2 * np.pi)) * np.exp(-1 / 2 * ((x - mu) / sigma) ** 2) + c
    )
...
def lorentzian_pol2(x, mu, sigma, A, c, p4, p5):
    return (
        A / np.pi * (1 / 2 * sigma) / ((x - mu) ** 2 + (1 / 2 * sigma) ** 2)
        + c
        + x * p4
        + x**2 * p5
    )
...
def gaussian_pol2(x, mu, sigma, A, c, p4, p5):
    return (
        A / (sigma * np.sqrt(2 * np.pi)) * np.exp(-1 / 2 * ((x - mu) / sigma) ** 2)
        + c
        + x * p4
        + x**2 * p5
    )
...
def auto_hist_fitting(f, bins, hist, p0=None):
    ...
    if f in ["gaussian", "lorentzian"]:
        ...
        popt, pcov = curve_fit(
            ...
            p0=(
                p0
                if p0 is not None
                else [0.0, 1.0, np.sum(hist) * width, 0, 0.015, 0.06]
            ),
            maxfev=2000,
        )
        fx = dict_f[f](x, *popt)
        ...
        popt, pcov = curve_fit(
            ...
            p0=(
                p0
                if p0 is not None
                else [0.0, 1.0, np.sum(hist) * width, 0.0, 0.5, np.sum(hist) * width, 0]
            ),
            maxfev=2000,
        )
        fx = dict_f[f](x, *popt)
    ...
    if data_slice.shape[0] > 1:
        ...
        try:
            ...
            popt, _ = curve_fit(
                ...
                y,
                p0=initial_guess,
                bounds=[(-200, 5, np.max(y) / 1.5, -2), (200, 150, 1.5 * np.max(y), 2)],
                maxfev=1000000,
            )
