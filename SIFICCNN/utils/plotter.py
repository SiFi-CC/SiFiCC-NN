...
def plot_score_distribution(y_scores, y_true, figure_name):
    ...
    ary_scores_pos = [
        float(y_scores[i]) for i in range(len(y_scores)) if y_true[i] == 1
    ]
    ary_scores_neg = [
        float(y_scores[i]) for i in range(len(y_scores)) if y_true[i] == 0
    ]
    ...
    h0, _, _ = plt.hist(
        np.array(ary_scores_pos), bins=bins, histtype="step", color="deeppink"
    )
    h1, _, _ = plt.hist(
        np.array(ary_scores_neg), bins=bins, histtype="step", color="blue"
    )
    plt.vlines(
        x=0.5,
        ymin=0.0,
        ymax=max([max(h0), max(h1)]),
        color="red",
...
def plot_efficiencymap(y_pred, y_true, y_sp, figure_name, theta=0.5, sr=100):
    ...
    for i in range(len(ary_eff)):
        if hist1[i] < 10:
            ...
            ary_eff_err[i] = np.sqrt(
                (np.sqrt(hist1[i]) / hist0[i]) ** 2
    ...
    axs[1].errorbar(
        ary_bin[:-1] + width, ary_eff, ary_eff_err, ary_bin_err, fmt=".", color="blue"
    )
...
def plot_roc_curve(list_fpr, list_tpr, figure_name, weighted=False):
    if weighted:
...
def plot_sp_distribution(ary_sp, ary_score, ary_true, figure_name):
    ...
    plt.hist(
        ary_sp[ary_sp != 0.0], bins=bins, color="orange", alpha=0.5, label="All events"
    )
...
def plot_pe_distribution(ary_pe, ary_score, ary_true, figure_name):
    # plot MC Source Position z-direction
    width = 0.1
...
def plot_position_error(y_pred, y_true, figure_name):
    ...
    h, _, _, _ = plt.hist2d(x=y_true[:, 0], y=y_pred[:, 0] - y_true[:, 0],
               bins=[bins_x, bins_err_x], norm=LogNorm())
    print(h)
    print(np.max(h))
    print(y_pred[:,0])
    print(np.max(y_pred[:,0]))
    # Print ranges and shapes
    print(f"y_true min: {y_true[:, 0].min()}, max: {y_true[:, 0].max()}")
    print(f"y_pred - y_true min: {(y_pred[:, 0] - y_true[:, 0]).min()}, max: {(y_pred[:, 0] - y_true[:, 0]).max()}")
    print(f"bins_x: {bins_x}")
    print(f"bins_err_x: {bins_err_x}")
    print(f"y_true shape: {y_true.shape}")
    print(f"y_pred shape: {y_pred.shape}")

    # Check if the bins cover the data range
    print(f"Do bins_x cover y_true range: {bins_x[0]} <= {y_true[:, 0].min()} <= {y_true[:, 0].max()} <= {bins_x[-1]}")
    print(f"Do bins_err_x cover error range: {bins_err_x[0]} <= {(y_pred[:, 0] - y_true[:, 0]).min()} <= {(y_pred[:, 0] - y_true[:, 0]).max()} <= {bins_err_x[-1]}")
    ...
    plt.ylabel("counts")
    plt.hist(
        y_pred[:, 1] - y_true[:, 1], bins=bins_err_y, histtype="step", color="blue"
    )
    plt.plot(
        ary_y,
        gaussian(ary_y, *popt1),
        color="deeppink",
        label=r"$\mu$ = {:.2f} $\pm$ {:.2f}"
        "\n"
        r"$\sigma$ = {:.2f} $\pm$ {:.2f}".format(
    ...
    plt.ylabel("counts")
    plt.hist(
        y_pred[:, 2] - y_true[:, 2], bins=bins_err_z, histtype="step", color="blue"
    )
    plt.plot(
        ary_z,
        gaussian(ary_z, *popt2),
        color="deeppink",
        label=r"$\mu$ = {:.2f} $\pm$ {:.2f}"
        "\n"
        r"$\sigma$ = {:.2f} $\pm$ {:.2f}".format(
    ...
    plt.ylabel("counts")
    plt.hist(
        y_pred[:, 3] - y_true[:, 3], bins=bins_err_x, histtype="step", color="blue"
    )
    plt.plot(
        ary_x,
        gaussian(ary_x, *popt3),
        color="deeppink",
        label=r"$\mu$ = {:.2f} $\pm$ {:.2f}"
        "\n"
        r"$\sigma$ = {:.2f} $\pm$ {:.2f}".format(
    ...
    plt.hist2d(
        x=y_true[:, 3],
        y=y_pred[:, 3] - y_true[:, 3],
        bins=[bins_x, bins_err_x],
        norm=LogNorm(),
    )
    plt.hlines(xmin=min(bins_x), xmax=max(bins_x), y=0, color="red", linestyles="--")
    # plt.colorbar()
    plt.grid(which="major", color="#DDDDDD", linewidth=0.8)
    plt.grid(which="minor", color="#EEEEEE", linestyle=":", linewidth=0.5)
    plt.minorticks_on()
    ...
    plt.ylabel("counts")
    plt.hist(
        y_pred[:, 4] - y_true[:, 4], bins=bins_err_y, histtype="step", color="blue"
    )
    plt.plot(
        ary_y,
        gaussian(ary_y, *popt4),
        color="deeppink",
        label=r"$\mu$ = {:.2f} $\pm$ {:.2f}"
        "\n"
        r"$\sigma$ = {:.2f} $\pm$ {:.2f}".format(
    ...
    plt.ylabel("counts")
    plt.hist(
        y_pred[:, 5] - y_true[:, 5], bins=bins_err_z, histtype="step", color="blue"
    )
    plt.plot(
        ary_z,
        gaussian(ary_z, *popt5),
        color="deeppink",
        label=r"$\mu$ = {:.2f} $\pm$ {:.2f}"
        "\n"
        r"$\sigma$ = {:.2f} $\pm$ {:.2f}".format(
...
def plot_theta_error(y_pred, y_true, figure_name):
    ...
    plt.ylabel("counts")
    plt.hist(y_pred - y_true, bins=bins_err, histtype="step", color="blue")
    plt.plot(
        ary_x,
        gaussian(ary_x, *popt0),
        color="orange",
...
def plot_position_error_vs_energy(pos_pred, pos_true, e_total, figure_name):
    ...
    plt.hist2d(
        x=e_total,
        y=pos_pred[:, 0] - pos_true[:, 0],
        bins=[bins_energy, bins_err_x],
        norm=LogNorm(),
    )
    plt.hlines(
        xmin=min(bins_energy), xmax=max(bins_energy), y=0, color="red", linestyles="--"
    )
    plt.colorbar(norm=LogNorm())
    plt.grid(which="major", color="#DDDDDD", linewidth=0.8)
    plt.grid(which="minor", color="#EEEEEE", linestyle=":", linewidth=0.5)
    ...
    plt.hist2d(
        x=e_total,
        y=pos_pred[:, 1] - pos_true[:, 1],
        bins=[bins_energy, bins_err_y],
        norm=LogNorm(),
    )
    plt.hlines(
        xmin=min(bins_energy), xmax=max(bins_energy), y=0, color="red", linestyles="--"
    ...
    plt.hist2d(
        x=e_total,
        y=pos_pred[:, 2] - pos_true[:, 2],
        bins=[bins_energy, bins_err_z],
        norm=LogNorm(),
    )
    plt.hlines(
        xmin=min(bins_energy), xmax=max(bins_energy), y=0, color="red", linestyles="--"
...
def plot_2dhist_ep_score(pe, y_score, y_true, figure_name):
    ...
    plt.ylabel("Score")
    h0 = plt.hist2d(
        pe[y_true == 1], y_score[y_true == 1], bins=[bin_pe, bin_score], norm=LogNorm()
    )
    plt.colorbar(h0[3], norm=LogNorm())
...
def plot_energy_resolution(y_pred, y_true, figure_name, mode):
    ...
    # method for determining an estimation of the full-width-half-maximum for
    # fitting
    ...
    # binning true energy
    # bin width increases for higher energies to increase statistics per
    # bin-range
    bins_energy_e = np.concatenate(
        [
            [0.0, 0.2],
    ...
    bin_energy_e_center = np.array(
        [
            bins_energy_e[i] + (bins_energy_e[i + 1] - bins_energy_e[i]) / 2
            for i in range(len(bins_energy_e) - 1)
        ]
    )
    bin_energy_e_center_err = np.array(
        [
            (bins_energy_e[i + 1] - bins_energy_e[i]) / 2
            for i in range(len(bins_energy_e) - 1)
        ]
    )
    ...
    plt.errorbar(
        bin_energy_e_center,
        ary_res_e / bin_energy_e_center * 100,
        ary_res_e_err / bin_energy_e_center * 100,
        bin_energy_e_center_err,
        fmt=".",
        color="deeppink",
        label=r"$E_e$",
    )
