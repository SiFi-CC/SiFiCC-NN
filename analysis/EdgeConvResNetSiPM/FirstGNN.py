...
from tensorflow.keras.layers import Input, Dense, Reshape, Flatten, AveragePooling1D, GlobalAveragePooling2D
...
def build_graph_network(input_dim, hidden_dim, output_dim):
    ...
    gc1 = spektral.layers.GraphSageConv(300,activation=None)([X_input, A_input])
    gc2 = spektral.layers.GraphSageConv(770,activation=None)([gc1, A_input])

    # Pooling or Readout layer (for simplicity, let's use global sum pooling)
    pool = spektral.layers.GlobalSumPool()(gc2)
